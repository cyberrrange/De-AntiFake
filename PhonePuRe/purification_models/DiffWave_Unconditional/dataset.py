...
def set_audio_length(data, time):
    ...
    if length < len(samples):
        ...
        # Calculate the range from which we can start sampling to achieve the exact length needed
        start = random.randint(0, len(samples) - length)
        # Slice the samples from a random start point
        data['samples'] = samples[start:start + length]
...
class WAVAudioDataset(Dataset):
    def __init__(self, root: str, transform=LoadAudio()) -> None:
        super().__init__()
        self.root = root
        self.transform = transform
        self.data = []
        ...
        # 定义需要排除的说话人列表
        excluded_speakers = {'1089', '1188', '1221', '1284', '1320', 
                             '1580', '1995', '2094', '2300', '2830', 
                             '2961', '3570', '3575', '3729', '4077', 
                             '4446', '4507', '4970', '4992', '5105', 
                             '5142', '5639', '5683', '6829', '6930'}
    ...
    def __getitem__(self, index):
        ...
        data = {'path': path, 'target': 0}  # 'target' 为0，因为没有分类任务
        if self.transform:
            data = self.transform(data)
        return data


def load_wav_files(path, batch_size=4, num_gpus=1):
    """
    Load dataset from a folder containing .wav files
    """
    wav_dataset = WAVAudioDataset(root=path)
    
    # 如果使用多GPU，则启用分布式采样
    train_sampler = DistributedSampler(wav_dataset) if num_gpus > 1 else None
    
    trainloader = torch.utils.data.DataLoader(wav_dataset, 
                             batch_size=batch_size,  
                             sampler=train_sampler,
                             collate_fn=rand_audio_length_collate, 
                             num_workers=4,
                             pin_memory=True,  # 当使用GPU时，推荐设置为True
                             drop_last=True)
    return trainloader
