...
def get_pc_sampler(
	...
	def pc_sampler():
		'''
		if predictor_name == 'one_shot_diffusion' or predictor_name == 'two_shot_diffusion':
			# Force single step
			sde.N = 30

			# Suppose 'y' is your conditioning or data shape reference
			xt = sde.prior_sampling(y.shape, y).to(y.device)
			#xt = y

			# Single time step from sde.T to eps
			timesteps = torch.linspace(sde.T, eps, sde.N, device=y.device)

			#for i in range(sde.N):  # only one iteration
			t = timesteps[0]
			vec_t = torch.ones(y.shape[0], device=y.device) * t

			# Optional corrector update; you can skip if you want purely one-step
			xt, _ = corrector.update_fn(xt, vec_t, y, conditioning=conditioning)

			# Single-step call to the new predictor
			xt, xt_mean = predictor.update_fn(xt, vec_t, y, conditioning=conditioning)

			# If you want the final denoised result
			#x_result = xt_mean
			x_result = xt
			# Count steps to track usage if needed
