...
def visualize_example(mix, estimate, target, denoised, idx_sample=0, epoch=0, name="", sample_rate=16000, hop_len=128, return_fig=False):
	...
	if isinstance(mix, torch.Tensor):
		...
		denoised = torch.abs(denoised).detach().cpu()
	...
	ax = axes.flat[2]
	ax.pcolormesh(frames, freqs, 20*np.log10(.1*denoised + EPS_graphics), vmin=vmin, vmax=vmax, shading="auto", cmap="magma")
	ax.set_xlabel('Time [s]')
	ax.set_ylabel('Frequency [Hz]')
	ax.set_title('Anechoic denoised')
