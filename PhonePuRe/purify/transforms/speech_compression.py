...
class CompressionDefense():
    def __init__(self, defense_type):
        self.defense_type = defense_type    
    def __call__(self, x, *args):

        if self.defense_type == 'OPUS':
            output = OPUS(x, *args)
        elif self.defense_type == 'SPEEX':
            output = SPEEX(x, *args)
        elif self.defense_type == 'AMR':
            output = AMR(x, *args)
        elif self.defense_type == 'AAC_V':
            output = AAC_V(x, *args)
        elif self.defense_type == 'AAC_C':
            output = AAC_C(x, *args)
        elif self.defense_type == 'MP3_V':
            output = MP3_V(x, *args)
        elif self.defense_type == 'MP3_C':
            output = MP3_C(x, *args)
