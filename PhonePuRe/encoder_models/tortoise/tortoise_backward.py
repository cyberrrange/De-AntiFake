...
# Get the directory of the current file to build relative paths correctly
_current_dir = os.path.dirname(os.path.abspath(__file__))
_ckpt_dir = os.path.join(_current_dir, '..', 'speaker_encoder_ckpts', 'tortoise')

# Verify paths exist (for debugging)
if not os.path.exists(_ckpt_dir):
    print(f"Warning: Checkpoint directory not found: {_ckpt_dir}")

DEFAULT_MEL_NORM_FILE = os.path.join(_ckpt_dir, 'mel_norms.pth')
BUILTIN_VOICES_DIR = os.path.join(_current_dir, 'voices')  # Relative to current file directory
AUTOREGRESSIVE_ENCODER = os.path.join(_ckpt_dir, 'autoregressive.pth')
DIFFUSION_ENCODER = os.path.join(_ckpt_dir, 'diffusion_decoder.pth')

# Verify critical model files exist
for path, name in [(AUTOREGRESSIVE_ENCODER, 'Autoregressive model'), 
                   (DIFFUSION_ENCODER, 'Diffusion model'),
                   (DEFAULT_MEL_NORM_FILE, 'MEL normalization file')]:
